...
let sendInfo = { "title": "", "value": "" };
export function generateSendTopicView(Container) {
    let value = "";
    sendInfo = { "title": "", "value": value };
  
    function updateValue(uValue) {
        value = uValue;
        sendInfo.value = uValue
    }
    function updateTitle(uTitle) {
        sendInfo.title = uTitle;
    }

    let UI =
        <div style={{ display: "flex", flexDirection: "column" }}>
            <div style={{ width: "100%" }}>标题<input onChange={updateTitle} /></div>
            <UbbEditor update={updateValue} value={value} />
    </div>;
    ReactDOM.render(UI, Container);
}
export function returnSendInfo() {
    return sendInfo;
}
/**
 * UBB编辑器组件
